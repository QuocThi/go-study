...
func NewIndex(client *elastic.Client, indexFormat string, loc *time.Location, logger *slog.Logger) *Index {
	return &Index{
		client:      client,
...
func (idx *Index) GetIndex(m map[string]interface{}) string {
	...
	if err := idx.ensureIndex(index); err != nil {
		idx.logger.Error("ensure index failed", "index", index, "error", err)
	}
	return index
}

func (idx *Index) ensureIndex(index string) error {
	_, err, _ := idx.sf.Do(index, func() (interface{}, error) {
		idx.lock.Lock()
		...
		ensureErr = retry(3, 200*time.Millisecond, func() error {
