...
The embedded gin server is enabled automatically and listens on the address specified by `HTTP.Addr` (defaults to `:8080`). Useful endpoints:

- `GET /healthz` – liveness/readiness probe
- `GET /clusters` – returns the configured Kafka->ElasticSearch pipelines
...
go-stash now ships with a lightweight HTTP control plane powered by [**gin**](https://github.com/gin-gonic/gin). Hit `/healthz` for health checks or `/clusters` to inspect the currently loaded pipelines.
